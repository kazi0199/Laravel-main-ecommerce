...
<main class="pt-90">
              ...
              <div class="search-field multi-select accordion-body px-0 pb-0">
                <ul class="list list-inline mb-0 brand-list">
                  @foreach ($brands as $brand)
                     <li class="list-item">
                        <span class="menu-link py-1">
                            <input type="checkbox" class="chk-brand" name="brands" value="{{ $brand->id }}"
      ...
      <div class="shop-list flex-grow-1">
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
            <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" id="pagesize" aria-label="Page Size"
                ...
                <div class="pc__info position-relative">
                    ...
                    @if(Cart::instance('wishlist')->content()->where('id',$product->id)->count()>0)
                      <form action="{{ route('wishlist.item.remove',Cart::instance('wishlist')->content()->where('id',$product->id)->first()->rowId) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 filled-heart" title="Remove From Wishlist">
  ...
  <form action="{{ route('shop.index') }}" id="frmfilter" method="GET">
    ...
    <input type="hidden" name="size" id="size" id="{{ $size }}">
    <input type="hidden" name="order" id="order" id="{{ $order }}">
    <input type="hidden" name="brands" id="hdnBrands">
    <input type="hidden" name="categories" id="hdnCategories">
    <input type="hidden" name="min" id="hdnMinPrice" id="{{ $min_price }}">
    <input type="hidden" name="max" id="hdnMaxPrice" id="{{ $max_price }}">
  </form>
@endsection
...
<script>
  $(function(){
    ...
    $("[name='price_range']").on("change input slideStop slide", function () {

        let value = $(this).val();

        // If returns "10,100"
        let arr = value.includes(',') ? value.split(',') : JSON.parse(value);
